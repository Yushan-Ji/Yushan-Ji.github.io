/**
 * 基于Tessendorf波实现的海水着色器
 * 参考源代码：http://david.li/waves
 * 
 * 使用了七个着色器：
 * 1. ocean_sim_vertex: 用于设置一个以(0,0)为中心的2x2模拟平面的顶点着色器
 * 2. ocean_subtransform: GPU FFT（Fast Fourier Transform）使用Stockham公式进行子转换的片段着色器，生成位移图
 * 3. ocean_initial_spectrum: 设置初始波频率的片段着色器
 * 4. ocean_phase: 设置波相位的片段着色器
 * 5. ocean_spectrum: 设置当前波频率的片段着色器
 * 6. ocean_normals: 设置面法线的片段着色器
 * 7. oceanfft: 最终的渲染着色器，包含顶点和片段着色器，用于创建最终的渲染效果
 */
...
// 计算面法线，用于rendering阶段模拟海平面的反射光照效果
...
THREE.UniformsLib[ "oceanfft" ] = {
	...
	"u_displacementMap": { type: "t", value: null }, // 位移贴图
	"u_reflection": { type: "t", value: null },      // 反射贴图
	"u_normalMap": { type: "t", value: null }, 
	"u_geometrySize": { type: "f", value: null },
	"u_size": { type: "f", value: null },
	"u_mirrorMatrix": { type: "m4", value: null },
	"u_cameraPosition": { type: "v3", value: null }, // 相机位置
	"u_skyColor": { type: "v3", value: null },       // 天空颜色
	"u_oceanColor": { type: "v3", value: null },     // 海水颜色
	"u_sunDirection": { type: "v3", value: null },   // 光源方向
